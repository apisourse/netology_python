if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[sp_CopyEDD]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[sp_CopyEDD]
GO
...
CREATE PROCEDURE [dbo].[sp_CopyEDD](@TT INT, @Copy INT, @Paste INT, @attributes as VARCHAR(50), @program as VARCHAR(50), @type as VARCHAR(50)) AS
...
BEGIN
    SELECT
      ...
      WHERE
        ...
        OR (IB.table_name = 'V_ELA' AND @attributes = 'ela')
        OR (IB.table_name = 'R_FIN_DS' AND @attributes = 'edd')          
        )
         ...
         AND (
		((SELECT EDD_Load_State FROM r_fin_ds where id_ds=@Paste)='In elaboration')
		OR  (
			((SELECT EDD_Load_State FROM r_fin_ds where id_ds=@Paste)='Proposed to validation')
			AND( object_name NOT IN (SELECT object_name FROM T_MG where eddType='LOAD'))
		       )
	)
