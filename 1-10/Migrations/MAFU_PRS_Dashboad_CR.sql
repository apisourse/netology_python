------------------------------------------------------------
-- Report PRS Dashboard by installer and procurement for CR
...
DELETE FROM T_DBActions WHERE label = 'PRS Dashboard by installer and procurement for CR' AND comments like '%- Report. Only for CR.%'
...
INSERT INTO T_DBActions (
	...
	report_group_idr) 
SELECT 
	'PRS Dashboard by installer and procurement for CR',
	'EXECUTE[Dashboard_Procurement_MP_CR]',
	'- Report. Only for CR.', 
	'Industrial_Dashboard_Procurement_CR.php', 
	null,
	null, 
	null, 
	report_group_id 
...
IF EXISTS (SELECT name FROM sysobjects WHERE name = 'Dashboard_Procurement_MP_CR' AND type = 'P')
	DROP PROCEDURE Dashboard_Procurement_MP_CR
GO


CREATE PROCEDURE Dashboard_Procurement_MP_CR AS
	Set concat_null_yields_null off
	
	SELECT  distinct 
		MPReference AS MP, 
		dbo.PRSConcatRequest_CR(id_mp) AS CR, 
		MPTitle AS [MP Title], 
	...
	WHERE     (ACMTLeaderMP = 'SYS') and (typeOfRequest='CR') 
