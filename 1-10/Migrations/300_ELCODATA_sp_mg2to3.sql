		...
		' +
			...
			CASE
				WHEN
					...
					AND (
						SELECT ISNULL(LoadGroup2,'''') FROM R_FIN_DS where id_ds = ' + CONVERT(VARCHAR(8000), @id_ds) + '
					) = ''''
				THEN
			...
			CASE
				WHEN
					...
					AND (
						...
						SELECT ISNULL(LoadGroup3,'''') FROM R_FIN_DS where id_ds = ' + CONVERT(VARCHAR(8000), @id_ds) + '
					) = ''''
				THEN
			...
			CASE
				WHEN
					PowerCenterType = ''SPDB''
					AND (
						SELECT ISNULL(LoadGroup4,'''') FROM R_FIN_DS where id_ds = ' + CONVERT(VARCHAR(8000), @id_ds) + '
					) = ''''
				THEN
