if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[sp_CopyAsNewEDD]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[sp_CopyAsNewEDD]
GO
...
CREATE PROCEDURE [dbo].[sp_CopyAsNewEDD]
(
	@ELA 		AS INT,
	@TT			AS INT,
	@Copy		AS INT,
...
AS
	...
	--
	-- @param		ELA			if = 1 => Copy also the ELA consumptions
	...
	BEGIN
		...
		------------------------------------------------------------------------
		-- Part 1 : copy EDD attributes (and also ELA if selected)
		------------------------------------------------------------------------
		SET @SQL =  'UPDATE p SET '
		SELECT
			@SQL = @SQL + CASE WHEN @SQL = 'UPDATE p SET ' THEN 'p.' ELSE ', p.' END + object_value + ' = c.' + object_value
		...
		AND object_value NOT IN ('DS_SI_Reference', 'NAME', 'AccesDoorPanel', 'ANSI_CODE', 'BasicOptional', 'Category', 'Comments',
				...
				'SVStatus', 'SystemViewStatus', 'TypeEqt', 'Version', 'WeightStatus','EDDCheckSum', 
				'VALM0', 'VALM1', 'VALM2')
		AND ((table_name = 'R_FIN_DS' AND @ELA = 0) OR (table_name IN ('V_ELA', 'R_FIN_DS') AND @ELA = 1))
		GROUP BY object_value
	...
	BEGIN
		...
		------------------------------------------------------------------------
		-- Copy EDD attributes (and ELA if selected)
		------------------------------------------------------------------------
		SET @SQL =  'UPDATE p SET '
