----Add DQN/RDR Number + Sketch  Attributes on WDDS(TSD-ESDCR_DM-550)
------------------------------------------------------------------------------------------------------------------------------------------------------------
...
BEGIN
INSERT INTO T_EDIP_ATTRIBUTE (ATTRIBUTE_ATTRIBUTE_GROUP_idr,AttributeLabel,AttributeName,TableName,SearchType,EditType,AttributeType,AttributeOrder,AttributeDescription,ValuesList,FieldSize,MaxLength,ReadOnly) VALUES (@ATTRIBUTE_ATTRIBUTE_GROUP_idr,'DQN/RDR Number','WDDSDQNNumber','T_WDDS','text','text','text',@AttributeOrder,'DQN/RDR Number',NULL,@FieldSize,@MaxLength,@ReadOnly)
END
...
SELECT @ATTRIBUTE__CONTEXT_ATTRIBUTE_idr=ATTRIBUTE_id FROM T_EDIP_ATTRIBUTE WHERE AttributeLabel='DQN/RDR Number' AND AttributeName='WDDSDQNNumber'
...
BEGIN
INSERT INTO T_EDIP_ATTRIBUTE (ATTRIBUTE_ATTRIBUTE_GROUP_idr,AttributeLabel,AttributeName,TableName,SearchType,EditType,AttributeType,AttributeOrder,AttributeDescription,ValuesList,FieldSize,MaxLength,ReadOnly) VALUES (@ATTRIBUTE_ATTRIBUTE_GROUP_idr,'Sketch Number','WDDSSketchNumber','T_WDDS','text','text','text',@AttributeOrder,'Sketch Number',NULL,@FieldSize,@MaxLength,@ReadOnly)
END
--Give access rights for the attribute "Sketch Number" to "Admin" Role
SELECT @ATTRIBUTE__CONTEXT_ATTRIBUTE_idr=ATTRIBUTE_id FROM T_EDIP_ATTRIBUTE WHERE AttributeLabel='Sketch Number' AND AttributeName='WDDSSketchNumber'
SET @AttributeContextReadMode=1
SET @AttributeContextWriteMode=0
IF(NOT EXISTS(SELECT * FROM T_EDIP_LINK_ATTRIBUTE__CONTEXT WHERE ATTRIBUTE__CONTEXT_ATTRIBUTE_idr=@ATTRIBUTE__CONTEXT_ATTRIBUTE_idr AND ATTRIBUTE__CONTEXT_CONTEXT_idr=@ATTRIBUTE__CONTEXT_CONTEXT_idr ))
BEGIN
...
DEALLOCATE WRITERRIGHTS_CURSOR
----------------------------------------------------------------------------------------------------------------------------------
--Adding column "WDDSDQNNumber" in table T_WDDS
IF (NOT EXISTS( SELECT * FROM INFORMATION_SCHEMA.COLUMNS 
            WHERE TABLE_NAME = 'T_WDDS' 
           AND  COLUMN_NAME = 'WDDSDQNNumber'))
BEGIN
ALTER TABLE T_WDDS ADD WDDSDQNNumber varchar(20)
END 
----------------------------------------------------------------------------------------------------------------------------------
--Adding column "WDDSSketchNumber" in table T_WDDS
IF (NOT EXISTS( SELECT * FROM INFORMATION_SCHEMA.COLUMNS 
            WHERE TABLE_NAME = 'T_WDDS' 
           AND  COLUMN_NAME = 'WDDSSketchNumber'))
BEGIN
ALTER TABLE T_WDDS ADD WDDSSketchNumber varchar(20)
END 
